...
//  backgrounds in the resonant di-Higgs analysis (ttbar, yybb, yybj, single- //
//  Higgs events).                                                            //
//                                                                            //
//  Associated config file: data/signalParam/diHiggsResBkg.cfg                //
//                                                                            //
////////////////////////////////////////////////////////////////////////////////
...
double getNTotEvtFromHist(TFile *f) {
  ...
  std::cout << "createDiHiggsResBkg: ERROR! MxAOD doesn't have cutflow"
	    << std::endl;
  exit(0);
}
...
std::vector<TString> makeLocalFileCopies(std::vector<TString> fileNames) {
  std::cout << "createDiHiggsResBkg: Making local copies of inputs."
	    << std::endl;
...
void removeLocalFileCopies(std::vector<TString> fileNames) {
  std::cout << "createDiHiggsResBkg: Removing local copies of inputs."
	    << std::endl;
  for (int i_f = 0; i_f < (int)fileNames.size(); i_f++) {
...
int main(int argc, char *argv[])
{
  ...
  std::cout << "createDiHiggsResBkg will run with parameters:"
	    << std::endl;
  settings->printDB();
  ...
  //sps->setMassWindowSize(settings->getNum("FitWindowFraction"));
  sps->setMassWindowFixed(true, settings->getNum("FitWindowMin"),
			  settings->getNum("FitWindowMax"));
  ...
  std::cout << "createDiHiggsResBkg: Start fitting and plotting!" 
	    << std::endl;
  ...
  std::cout << "createDiHiggsResBkg: Loop over the categories."
	    << std::endl;
  for (int i_c = 0; i_c < nCategories; i_c++) {
    ...
    if (sps->makeSingleResonance(resonanceMass, i_c, function)) {
      ...
      std::cout << "createDiHiggsResBkg: Fit at mRes=" << resonanceMass 
  ...
  std::cout << "createDiHiggsResBkg: Printing inclusive yields from counting"
	    << std::endl;
  ...
  std::cout << "createDiHiggsResBkg: Printing yields in each category from tool"
	    << std::endl;
