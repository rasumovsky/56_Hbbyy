...
int main(int argc, char *argv[])
{
  ...
  std::vector<double> fitNorms; fitNorms.clear();
  std::vector<double> dataNorms; dataNorms.clear();
  ...
  for (int i_t = 0; i_t < settings->getInt("NumberOfToys"); i_t++) {
    ...
    if (spt->generateAndFitData(mass, 2, "mctoy", seed+i_t)) {
      // Retrieve parameter valuess for the fit to this toy:
      for (int i_p = 0; i_p < (int)pars.size(); i_p++) {
	storeVals[pars[i_p]]
	  .push_back(spt->getParameterValue(pars[i_p], mass, category));
      }
      // Also save normalization data:
      fitNorms.push_back(spt->extendedTerm());
      dataNorms.push_back(spt->generatedDataNorm());
    }
    delete spt;
  }
  ...
  for (int i_p = 0; i_p < (int)pars.size(); i_p++) {
    ...
    currHist->GetYaxis()->SetRangeUser(0, (1.3 * currHist->GetMaximum()));
    can->cd(); 
    can->Clear();
    currHist->Draw("hist");
    ...
    // Print the canvas:
    can->Print(Form("%s/Bias/biasDist_%s_Mass%2.2f.eps", outputDir.Data(), 
		    pars[i_p].Data(), mass));
  }
  
  // Then plot the bias on the normalization:
  double fitAvg = 0.0; double fitMin = 0.0; double fitMax = 0.0;
  getAvgMinMax(fitNorms, fitAvg, fitMin, fitMax);
  double dataAvg = 0.0; double dataMin = 0.0; double dataMax = 0.0;
  getAvgMinMax(dataNorms, dataAvg, dataMin, dataMax);
  
  TH1F *hFitNorm = new TH1F("hFitNorm", "hFitNorm", 50, fitMin, fitMax);
  TH1F *hDataNorm = new TH1F("hDataNorm", "hDataNorm", 50, dataMin, dataMax);
  for (int i_e = 0; i_e < (int)fitNorms.size(); i_e++) {
    hFitNorm->Fill(fitNorms[i_e]);
    hDataNorm->Fill(dataNorms[i_e]);
  }
  hFitNorm->GetXaxis()->SetTitle("signal yield");
  hFitNorm->SetFillColor(kBlue-7);
  hFitNorm->SetFillStyle(3345);
  hFitNorm->SetLineColor(kBlue+1);
  hDataNorm->SetFillColor(kRed-7);
  hDataNorm->SetFillStyle(3354);
  hDataNorm->SetLineColor(kRed+1);
  hFitNorm->GetYaxis()->SetRangeUser(0, (1.3 * hFitNorm->GetMaximum()));
  can->cd(); 
  can->Clear();
  hFitNorm->Draw("hist");
  hDataNorm->Draw("histSAME");
  ...
  lFitAvg->DrawLine(fitAvg, 0, fitAvg, hFitNorm->GetMaximum());
  // Draw origin value:
  TLine *lDataAvg = new TLine();
  lDataAvg->SetLineStyle(2);
  lDataAvg->SetLineWidth(2);
  lDataAvg->SetLineColor(kRed+1);
  lDataAvg->DrawLine(dataAvg, 0, dataAvg, hDataNorm->GetMaximum());
  ...
  leg.AddEntry(lDataAvg, Form("Data Mean = %2.3f", dataAvg), "L");
  leg.Draw("SAME");
    
  can->Print(Form("%s/Bias/biasDist_Normalization_Mass%2.2f.eps",
		  outputDir.Data(), mass));
